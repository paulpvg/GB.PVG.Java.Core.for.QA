...
public class Child implements Athlete{

    String name;
    private final int maxDistanceRun = 500;
    private final int maxDistanceSwim = 0; // Ребетёнки плавать не умеют
    private final int maxHeightJump = 2;
    boolean onDistance;
    ...
    public void swim(int distance) {
        if (distance == 0) {
            System.out.println(name + " увидел, что река пересохла, обрадовался и побежал дальше");
        }
        else {
    ...
    public void showMembers() {
        System.out.println(name + " - может пробежать " + maxDistanceRun + "м и подпрыгнуть на " + maxHeightJump + "м, а вот плавать совсем не умеет.");
    }
